...
- **发版前**（大版本必须，小版本建议）：运行 `scripts/release.sh <x.y.z>` 自动完成以下动作（或手动）：把 `## [Unreleased]` 改名为 `## [x.y.z] - YYYY-MM-DD`，确保段内有 `### What's New` —— 用面向球友的自然语言描述新功能和变化，不写技术实现细节；顶部新开一个空的 `## [Unreleased]`；同步 `web/package.json` 的 `version`。
